...
class ApiTest(TestCase):
    ...
    def test_api_berechnung_with_missing_data(self):
        """Test the api_berechnung view with missing data."""
        # Make the API request with no parameters
    ...
    def test_api_berechnung_with_partial_data(self):
        """Test the api_berechnung view with partial data."""
        # Prepare query parameters with only some values
        params = {
            'laenge': 20.0,
            'breite': 15.0,
            'geschosshoehe': 3.0,
        ...
        # Check that energy values are 0 (since no energy data was provided)
        nutzenergie = data['nutzenergie']
        self.assertAlmostEqual(nutzenergie['ne_absolut'], 0.0, delta=0.1)
